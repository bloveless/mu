...
impl HistoryItem {
    ...
    fn wrapped_lines(&self) -> &[String] {
        match self {
            ...
            | HistoryItem::AssistantThinking(w)
...
pub struct App {
    ...
    streaming_thinking_item: Option<HistoryItem>,
    streaming_content: String,
    streaming_content_item: Option<HistoryItem>,
...
impl App {
    ...
    fn handle_event(&mut self, event: AppEvent) -> Result<bool> {
        match event {
            ...
            AppEvent::ChunkReceived(response) if !response.is_empty() => {
                self.streaming_content.push_str(&response);
                self.streaming_content_item = Some(HistoryItem::AssistantResponse(
                    WrappedItem::new(self.streaming_content.clone()),
                ));
                self.cached_total_height_width = None;
                self.scroll_view_dirty = true;
            ...
            AppEvent::ThinkingChunkReceived(thinking) if !thinking.is_empty() => {
                self.streaming_thinking.push_str(&thinking);
                self.streaming_thinking_item = Some(HistoryItem::AssistantThinking(
                    WrappedItem::new(self.streaming_thinking.clone()),
                ));
                self.cached_total_height_width = None;
                self.scroll_view_dirty = true;
                self.scroll_to_bottom_if_following();
            ...
            AppEvent::TurnEnd => {
                if !self.streaming_thinking.is_empty() {
                    self.history
                        .push(HistoryItem::AssistantThinking(WrappedItem::new(
                ...
                self.streaming_content_item = None;
                self.streaming_thinking_item = None;
            ...
            AppEvent::Fatal(msg) => Err(anyhow::anyhow!(msg)),
            // Non-empty guard above catches the non-empty case; empty
            // responses / errors are no-ops.
            AppEvent::ChunkReceived(_)
            | AppEvent::ThinkingChunkReceived(_)
            | AppEvent::Error(_) => Ok(false),
        }
    }
    ...
    fn ensure_all_wrapped(&mut self, wrap_width: u16) -> u16 {
            ...
            && let HistoryItem::AssistantThinking(wrapped) = streaming_thinking_item
        {
            ...
            h = h.saturating_add(streaming_thinking_item.rendered_height());
        }
        // Append height of streaming content
        if let Some(streaming_content_item) = &mut self.streaming_content_item
            && let HistoryItem::AssistantResponse(wrapped) = streaming_content_item
        {
            wrapped.ensure_wrapped_at(wrap_width);
            h = h.saturating_add(streaming_content_item.rendered_height());
        }
    ...
    fn rebuild_scroll_view(&mut self, area_width: u16, _wrap_width: u16, size: Size) {
        ...
        for item in self
            .history
            .iter()
            .chain(self.streaming_thinking_item.iter())
            .chain(self.streaming_content_item.iter())
        {
            match item {
                HistoryItem::UserPrompt(w) => {
